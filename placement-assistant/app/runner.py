...
def call_tool(tools: PlacementTools, placement: PlacementDb, key: str, name: str, args: dict) -> dict:
    """Run one tool safely. Prevents repeated execution of side effects on crashed runs."""
    try:
        if name in tools.SIDE_EFFECTS:
            # placement.once returns (result_dict, executed_now_boolean)
            result, executed_now = placement.once(key, name, lambda: tools.call(name, args))
            return result
        else:
...
def execute_run(claimed: Claimed, *, store: RunStore, placement: PlacementDb, tools: PlacementTools, provider,
    ...
    while True:
        for step_seq, call in pending:
            ...
            # An already applied result is still a successful run step execution
            ok = "error" not in result or result.get("already_applied") is True
