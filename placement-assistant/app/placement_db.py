...
class PlacementDb:
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        """Insert an application or return an existing one safely in one transaction."""
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        """Atomically claim an interview slot verifying the version sequence match."""
        with self.transaction() as conn:
            row = conn.execute(
                "SELECT version, student_id FROM interview_slot WHERE id = ?", 
                (slot_id,)
            ).fetchone()
            
            if row is None or row["version"] != expected_version or row["student_id"] is not None:
                return False
            
            cur = conn.execute(
                "UPDATE interview_slot SET student_id = ?, version = version + 1 WHERE id = ? AND version = ?",
                (student_id, slot_id, expected_version)
            )
            return cur.rowcount > 0

    def book_interview_slot(self, slot_id: int, student_id: str, version: int) -> dict:
        """Atomically assign a slot to a student using a strict version verification lock."""
        with self.transaction() as conn:
            row = conn.execute("SELECT version, student_id FROM interview_slot WHERE id = ?", (slot_id,)).fetchone()
            if row is None:
                return {"status": "error", "message": "Slot not found"}
            if row["student_id"] == student_id:
                return {"status": "already_booked"}
            if row["version"] != version or row["student_id"] is not None:
                return {"status": "slot_lost"}
                
            conn.execute(
                "UPDATE interview_slot SET student_id = ?, version = version + 1 WHERE id = ? AND version = ?",
                (student_id, slot_id, version)
            )
            return {"status": "booked"}

    def notify_student(self, student_id: str, text: str) -> dict:
        """Insert a student notification event log row into the table."""
        with self.transaction() as conn:
            cur = conn.execute(
                "INSERT INTO notification (roll_no, message, created_at) VALUES (?, ?, ?)",
                (student_id, text, self.clock())
            )
            return {"status": "sent", "notification_id": cur.lastrowid}

    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        """Returns (notification_id, created) preventing duplicate execution runs."""
        with self.transaction() as conn:
            row = conn.execute(
    ...
    def list_applications(self, student_id: int) -> list[dict]:
        """Fetch all submitted applications along with any booked interview timestamps."""
        rows = self.conn.execute(
            """SELECT a.id AS application_id, a.drive_id, c.name AS company, d.role, d.status, 
                      a.created_at, i.starts_at AS interview_at
                 FROM application a
                 JOIN drive d ON d.id = a.drive_id
                 JOIN company c ON c.id = d.company_id
                 LEFT JOIN interview_slot i ON i.drive_id = a.drive_id AND i.student_id = a.student_id
                WHERE a.student_id = ? ORDER BY a.created_at ASC""",
            (student_id,)
        ).fetchall()
        ...
        return [
            {
                ...
                "interview_at": _dt(r["interview_at"]) if r["interview_at"] is not None else None
            }
            for r in rows
        ]

    def once(self, key: str, tool_name: str, effect: Callable[[], dict]) -> tuple[dict, bool]:
        """Run `effect` at most once per key. Safe against intermediate worker crashes."""
        row = self.conn.execute("SELECT result FROM idempotency WHERE key = ?", (key,)).fetchone()
        if row is not None:
